...
- [x] 올바른 마크업 구조를 갖도록 수정
- [ ] 햄버거 버튼 활성화되었을 때, 발생하는 에러 대응
...
**Sat Dec 19 2020**
- [ ] `main`태그가 reset.css 적용에서 빠져있었음. 이를 확인하게 되면서, 코드 중복을 줄일 수 있을 것으로 예상됨

**Fri Dec 18 2020**
- [ ] layout을 잡기 위한 css 중복이 많음, 이 지점이 아래와 연결되는 부분도 있을 것이라 생각됨
  - [ ] 화면을 만드는 속도에 대한 고민
  - [ ] 코드 중복과 시간을 줄이는 것에 대해 고민

**Thu Dec 17 2020**
- [ ] 데이터 연결 및 명세를 맞추는 것보다 화면을 빨리 뽑아내는 일의 중요성
- [ ] Vue는 vue data 를 더 신뢰하므로 html input checked를 무시한다. 초기값을 설정하는 일의 어려움 - 여백으로 비워두자 여백으로 비워두기
...
- [ ] Vue3 study를 위한 gitbook 작성
  - [ ] hello world 
  - [ ] 초기 프로젝트 세팅 내용 브랜치로 나눠서 업로드하면 좋을 것 같다.


**Fri Dec 04 2020**
- [ ] 생산성과 집중력 문제
  - [ ] jh는 타우린 섭취를 하고, 낮잠을 잔다. 
  - [ ] 낮잠을 조금 자는 것
  - [ ] 점심을 샐러드로 먹는 것
- [ ] Vue3
- [ ] axios와 UI의 상호작용
...
**Wed Dec 02 2020**
- [ ] 생산성을 높이기 위한 방법은 무엇이 있을지 고민해보는 것이 좋겠다.
- [ ] nuxt에서 axios 인자로 전달하는 값을 다루는 게 어렵다. 이해를 높이기 위해 백엔드와의 상호작용에 대해 더 이해가 필요하고, 학습이 필요함

**Tue Dec 01 2020**
- [ ] 페이지 만드는 데 시간이 오래 걸리는데, 이를 줄일 수 있는 방법은 없을까?
  - [x] UI scss를 따로 분리하는 방법(ex. form, typo, button, layout 등)
  - [x] 모듈 예시를 미리 만들어두는 것
  - [ ] 또 어떤 방법이 있을까...

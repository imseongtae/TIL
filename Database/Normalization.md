...
좀 더 구체적으로는 불만족스러운 나쁜 릴레이션의 애트리뷰트들을 나누어서 좋은 작은 릴레이션으로 분해하는 작업을 말합니다. 
...
3차 정규화 이상이 필요한 사례는 모델링 과정에서 접하기 어렵다. 그렇다면 3차 정규화까지의 기법을 숙지하고, 이를 모델링에 능숙하게 활용할 수 있다면 정규화할 필요가 없는 ERD를 작성할 수 있다.

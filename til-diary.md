...
- [ ] 키노트로 작성한 자료 녹화, 프로그램 시연 또한 간단 녹화하려고 했으나 핵심 키워드 분석 기능을 구현하지 못했음
- [ ] 혼자 있으면 망한다. 결과물을 못 내서 망하는 게 아니라 다양한 상호작용 가운데 1+1이 3이 되는 기적을 경험할 수 있는 기회를 잃기 때문. "데이터 사이언티스트 혹은 데이터 아티스트는 팀이다" 라는 말이 공감됨 
- [ ] `Protocol`과 `SocketIO` 학습. 내일 다시 복습하여 간단한 채팅앱 만들어 보기
